      ...
      date_default_timezone_set('Asia/Kolkata');
        if($request->isMethod('post')){
            $rules = array(
               ...
               $input = $request->input();
               $data['type']='global';
               $findchallenge = DB::table('challenges')->where('entryfee',$input['entryfee'])->where('win_amount',$input['win_amount'])->where('maximum_user',$input['maximum_user'])->where('contest_cat',$input['contest_cat'])->first();
          ...
          if(isset($input['confirmed_challenge'])){
            ...
            if($input['contest_type']=='Amount' && $input['pricecard_type']=='Percentage'){
              $data['confirmed_challenge'] = 1;
            }
          }
          if(isset($input['is_running'])){
  ...
  public function create_custom_contest(){
      ...
      if($f_type==''){
        ...
        if(request()->has('matchid')){
          ...
          if($matchid!=""){
            $allchallenges = DB::table('matchchallenges')->where('matchkey',$matchid)->where('fantasy_type',$f_type)->orderBy('is_private','ASC')->get();
          }
        }
        $currentdate = date('Y-m-d h:i:s');
  ...
  public function importdata($id){
    date_default_timezone_set('Asia/Kolkata');
     $findmatch = DB::table('listmatches')->where('matchkey',$id)->first();
        ...
        $f_type = request()->get('fantasy_type');
        $data['fantasy_type'] = !empty($f_type) ? $f_type : 'Cricket';
          $findleauges = DB::table('challenges')->where('freez',0)->where('fantasy_type',$data['fantasy_type'])->get();
          $charray = array();
