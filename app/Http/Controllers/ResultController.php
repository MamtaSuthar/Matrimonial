...
// result section
        ...
        if (!empty($aaa)) {
            foreach ($matchplayers as $row) {
                ...
                if ($match == 't20') {
                    ...
                    if ($findplayerrole != 'bowler') {
                        if ($bballs >= 5) {
                            if ($strikerate >= 90 && $strikerate <= 99.99) {
                                $strikePoint = -2;
                            } else if ($strikerate >= 80 && $strikerate <= 89.99) {
                                $strikePoint = -4;
    ...
    public function prediction_point($matchkey){
            $alldata = DB::connection('mysql')->table('joinpredictionteam')->where("matchkey",$matchkey)->get();
            foreach($alldata as $result){
                $actualdata = DB::connection('mysql')->table('matchover')->where("matchkey",$matchkey)->get();
                // dd($alldata1);
                foreach($actualdata as $actdata){
            if(($actdata->over >= 1 && $actdata->over <= 5)){
                $my_array = json_decode($result->overdata,true);
                foreach($my_array as $ovdata){
                    // dd($ovdata);
                    $data =  $actdata->run - $ovdata['runs'];
                    $teampoint = 10 - $data;
                    if($teampoint < 0){
                        $dataa['points'] = $result->points;
                    }
                    else{
                    $dataa['points'] = $teampoint + $result->points;
                    }
                }
                $a = DB::connection('mysql2')->table('joinpredictionteam')->where('matchkey', $matchkey)->update($dataa);
                // dd("jhjh");
            }
           else if(($actdata->over >= 6 && $actdata->over <= 10)){
            $my_array = json_decode($result->overdata,true);
            foreach($my_array as $ovdata){
                // dd($ovdata);
                $data =  $actdata->run - $ovdata['runs'];
                ...
                $data['points'] = $teampoint + $result->points;
                }
            }
            DB::connection('mysql2')->table('joinpredictionteam')->where('matchkey', $matchkey)->update($data);
            }
            else if(($actdata->over >= 11 && $actdata->over <= 15)){
                $my_array = json_decode($result->overdata,true);
                foreach($my_array as $ovdata){
                    // dd($ovdata);
                    $data =  $actdata->run - $ovdata['runs'];
                ...
                DB::connection('mysql2')->table('jointeam')->where('matchkey', $matchkey)->where('userid',$userid)->update($data);
            }
            else if(($findover->overs1 >= 16 && $findover->overs1 <= 20)){
                $my_array = json_decode($result->overdata,true);
                foreach($my_array as $ovdata){
                    // dd($ovdata);
                    $data =  $actdata->run - $ovdata['runs'];
...
// dd($match_key);
                                            ...
                                            if ($batting['ball_of_dismissed']['wicket_type'] == 'runout') {
                                                if (isset($ball_of_dismissed['other_fielder'])) {
                                                    if ($ball_of_dismissed['other_fielder'] != "") {
                                                        $throwerarray[] = $ball_of_dismissed['other_fielder'];
                                                    }
                                                }
                                                if (isset($ball_of_dismissed['fielder'])) {
                                                    if ($ball_of_dismissed['fielder']['key'] != "") {
                                                        $hitterarray[] = $ball_of_dismissed['fielder']['key'];
                                                        /* check if the player is hitter and thrower both */
                                                        if ($ball_of_dismissed['other_fielder'] == "") {
                                                            $throwerarray[] = $ball_of_dismissed['fielder']['key'];
                                                        }
                                                    }
                                                }
                                            }
    ...
    public function refund_amount()
    {
        ...
        $match_time = date('Y-m-d H:i:s', strtotime('-10 minutes', strtotime($current)));
                ...
                foreach ($allchallenges as $challenge) {
                    ...
                    if (!empty($joinedusers)) {
                        ...
                        if ($challenge->contest_type == 'Amount') {
                            ...
                            $prc_arr = array();
                            for ($i = 0; $i < $toWin; $i++) {
                                $prc_arr[$i + 1] = $challenge->win_amount / $toWin;
                            }
                        }
