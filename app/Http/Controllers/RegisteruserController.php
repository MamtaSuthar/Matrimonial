...
class RegisteruserController extends Controller
{
    ...
    public function userswallet()
    {
        ...
        $titles = $query->join('userbalance', 'userbalance.user_id', '=', 'registerusers.id')->orderBY('registerusers.id', 'DESC')->select(DB::raw('sum(userbalance.balance) as bal_sum'), DB::raw('sum(userbalance.winning) as win_sum'), DB::raw('sum(userbalance.bonus) as bonus_sum'))->first();
        ...
        $total = $win_sum + $bonus_sum + $bal_sum;

        return view('user.userswallet', compact('win_sum', 'bonus_sum', 'bal_sum', 'total'));
    }
    ...
    public function edituserdetails(Request $request, $id)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->file('image')) {
                ...
                $data['image'] = $geturl . 'public/uploads/user-profile/' . $data['image'];
                // delete old image
...
// ==================================================================================================================
    ...
    {
        return view('user.viewtransactions', compact('uid'));
    }
    ...
    public function viewtransactions_table($uid, Request $request)
    {
        ...
        if (request()->has('transaction_id')) {
            ...
            if ($transaction_id != "") {
                $query->where('transactions.transaction_id', 'LIKE', '%' . $transaction_id . '%');
            }
        }
        if (request()->has('cid')) {
            $cid = request('cid');
